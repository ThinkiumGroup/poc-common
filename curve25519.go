...
func GetPublicKey(privateKey []byte) [32]byte {
	...
	keygen(publicKey[:], nil, privateKey)
	return publicKey
}

func GetSharedSecret(privateKey []byte, publicKey []byte) [32]byte {
	var dhSharedSecret [32]byte
	curve(dhSharedSecret[:], privateKey, publicKey)
...
func SignMsg(message []byte, privateKey []byte) [64]byte {
	...
	digest := sha256.New()
	keygen(publicKey[:], sharedKey[:], privateKey)
	messageDigest := Sum256(message)
	digest.Reset()

	digest.Write(messageDigest[:])
	digest.Write(sharedKey[:])
	x := make([]byte, 32)
	x = digest.Sum(x[:0])
	digest.Reset()
	var y [32]byte
	...
	sign(signature[:], h, x[:], sharedKey[:])
	copy(signature[32:], h[0:32])
	return signature
}

func VerifyMsg(message []byte, signature []byte, publicKey []byte, enforceCanonical bool) bool {
	if enforceCanonical && !isCanonicalSignature(signature) {
		return false
	}

	if enforceCanonical && !isCanonicalPublicKey(publicKey) {
		return false
	}

	Y := make([]byte, 32)
	v := make([]byte, 32)
	copy(v[0:], signature[0:32])
	h := make([]byte, 32)
	copy(h[0:], signature[32:64])
	verify(Y[:], v[:], h[:], publicKey)
...
func keygen(P []byte, s []byte, k []byte) {
	clamp(k)
	core(P, s, k, nil)
}
 ...
 */
func curve(Z []byte, k []byte, P []byte) {
	core(Z, nil, k, P)
}
 ...
 */
func sign(v []byte, h []byte, x []byte, s []byte) bool {
	// v = (x - h) s  mod q
 ...
 */
func verify(Y []byte, v []byte, h []byte, P []byte) {
	/* Y = v abs(P) + h G  */
	var d [32]byte
	p := []long10{newLong10(), newLong10()}
	s := []long10{newLong10(), newLong10()}
	yx := []long10{newLong10(), newLong10(), newLong10()}
	yz := []long10{newLong10(), newLong10(), newLong10()}
	t1 := []long10{newLong10(), newLong10(), newLong10()}
	t2 := []long10{newLong10(), newLong10(), newLong10()}
	...
	/* and go for it! */
	for i = 31; i >= 0; i-- {
		vi = int((int32(vi) << 8) | (int32(v[i]) & 0xFF))
		hi = int((int32(hi) << 8) | (int32(h[i]) & 0xFF))
		di = int((int32(di) << 8) | (int32(d[i]) & 0xFF))
		...
		for j = 7; j >= 0; j-- {
			...
			mont_add(&t1[1], &t2[1], &t1[k], &t2[k], &yx[1], &yz[1], &p[(di>>uint(j))&1])
			mont_add(&t1[2], &t2[2], &t1[0], &t2[0], &yx[2], &yz[2], &s[(((vi^hi)>>uint(j))&2)>>1])
		}
	}
...
func unpack(x *long10, m []byte) {
	x._0 = (int64(m[0]) & 0xFF) | (int64(m[1])&0xFF)<<8 | (int64(m[2])&0xFF)<<16 | ((int64(m[3])&0xFF)&3)<<24
	x._1 = ((int64(m[3])&0xFF) & ^int64(3))>>2 | (int64(m[4])&0xFF)<<6 | (int64(m[5])&0xFF)<<14 | ((int64(m[6])&0xFF)&7)<<22
	x._2 = ((int64(m[6])&0xFF) & ^int64(7))>>3 | (int64(m[7])&0xFF)<<5 | (int64(m[8])&0xFF)<<13 | ((int64(m[9])&0xFF)&31)<<21
	x._3 = ((int64(m[9])&0xFF) & ^int64(31))>>5 | (int64(m[10])&0xFF)<<3 | (int64(m[11])&0xFF)<<11 | ((int64(m[12])&0xFF)&63)<<19
	x._4 = ((int64(m[12])&0xFF) & ^int64(63))>>6 | (int64(m[13])&0xFF)<<2 | (int64(m[14])&0xFF)<<10 | (int64(m[15])&0xFF)<<18
	x._5 = (int64(m[16]) & 0xFF) | (int64(m[17])&0xFF)<<8 | (int64(m[18])&0xFF)<<16 | ((int64(m[19])&0xFF)&1)<<24
	x._6 = ((int64(m[19])&0xFF) & ^int64(1))>>1 | (int64(m[20])&0xFF)<<7 | (int64(m[21])&0xFF)<<15 | ((int64(m[22])&0xFF)&7)<<23
	x._7 = ((int64(m[22])&0xFF) & ^int64(7))>>3 | (int64(m[23])&0xFF)<<5 | (int64(m[24])&0xFF)<<13 | ((int64(m[25])&0xFF)&15)<<21
	x._8 = ((int64(m[25])&0xFF) & ^int64(15))>>4 | (int64(m[26])&0xFF)<<4 | (int64(m[27])&0xFF)<<12 | ((int64(m[28])&0xFF)&63)<<20
	x._9 = ((int64(m[28])&0xFF) & ^int64(63))>>6 | (int64(m[29])&0xFF)<<2 | (int64(m[30])&0xFF)<<10 | (int64(m[31])&0xFF)<<18
}

/* Check if reduced-form input >= 2^255-19 */
func is_overflow(x long10) bool {
 ...
 * to the same buffer. */
func mul_small(xy *long10, x long10, y int64) *long10 {
	var t int64
	t = x._8 * y
	xy._8 = t & ((1 << 26) - 1)
	t = (t >> 26) + (x._9 * y)
...
func is_negative(x long10) int {
	a1 := 0
	if is_overflow(x) || x._9 < 0 {
		a1 = 1
	}
	return a1 ^ int(x._0&int64(1))
}
...
/* t1 = ax + az
 * t2 = ax - az  */
func mont_prep(t1 *long10, t2 *long10, ax long10, az long10) {
	add(t1, ax, az)
	sub(t2, ax, az)
}
...
func mont_add(t1 *long10, t2 *long10, t3 *long10, t4 *long10, ax *long10, az *long10, dx *long10) {
	mul(ax, *t2, *t3)
	mul(az, *t1, *t4)
	add(t1, *ax, *az)
	sub(t2, *ax, *az)
 ...
 *  X(Q) = (t3+t4)/(t3-t4)
 * clobbers t1 and t2, preserves t3 and t4  */
func mont_dbl(t1 *long10, t2 *long10, t3 *long10, t4 *long10, bx *long10, bz *long10) {
	sqr(t1, *t3)
	sqr(t2, *t4)
	mul(bx, *t1, *t2)
	sub(t2, *t1, *t2)
 ...
 * t is a temporary  */
func x_to_y2(t *long10, y2 *long10, x long10) {
	sqr(t, x)
...
/* P = kG   and  s = sign(P)/k  */
func core(Px []byte, s []byte, k []byte, Gx []byte) {
	dx := newLong10()
	t1 := newLong10()
	t2 := newLong10()
	t3 := newLong10()
	t4 := newLong10()
	x := []long10{newLong10(), newLong10()}
	z := []long10{newLong10(), newLong10()}
